...
class _ReceiptScreenState extends State<ReceiptScreen> {
  ...
  Future<void> _saveTransaction() async {
    if (_formKey.currentState!.validate()) {
      if (_selectedCreditAccount == null || _selectedDebitAccount == null || _selectedCurrency == null) {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حفظ سند القبض بنجاح')));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('سند قبض'), backgroundColor: Colors.green, foregroundColor: Colors.white),
                  ...
                  child: Padding(
                    ...
                    child: Form(
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          children: [
                            DropdownButtonFormField<String>(value: _selectedCreditAccount, items: _accountItems, onChanged: (val) => setState(() => _selectedCreditAccount = val), decoration: const InputDecoration(labelText: 'من حساب (الدائن)', border: OutlineInputBorder())),
                            const SizedBox(height: 16),
                            DropdownButtonFormField<String>(value: _selectedDebitAccount, items: _fundItems, onChanged: (val) => setState(() => _selectedDebitAccount = val), decoration: const InputDecoration(labelText: 'إلى حساب (الصندوق)', border: OutlineInputBorder())),
                            const SizedBox(height: 16),
                            Row(
                              children: [
                            ...
                            _isLoading ? const CircularProgressIndicator() : ElevatedButton.icon(onPressed: _saveTransaction, icon: const Icon(Icons.save), label: const Text('حفظ'), style: ElevatedButton.styleFrom(backgroundColor: Colors.green, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 50, vertical: 15))),
                          ],
                        ),
                      ),
                    ),
                        ...
                        child: StreamBuilder<List<ts.Transaction>>(
                          ...
                          builder: (context, snapshot) {
                            ...
                            if (!snapshot.hasData || snapshot.data!.isEmpty) {
                              return const Center(child: Text('لا توجد سندات قبض لعرضها'));
                            }
